...
El objetivo es encontrar grupos de jugadores con similares números estadísticos para variables de ataque y defensa, y su peso y altura. Después, trataremos de relacionar los clusters obtenidos con la variable *salary*.

Puesto que los datos están medidos en distintas unidades: conteo de instancias de cada suceso por los jugadores, años jugados y altura y peso, hace falta escalar los datos.
...
Se utilizará como medida de distancia la distancia euclídea, ya que en este caso nos interesa agrupar jugadores con valores similares en ciertas estadísticas de performance, y no con perfiles similares en dichos parámetros. La agrupación por perfiles similares la podemos hacer más adelante.
...
Observamos a simple vista entre 6 y 8 clusters, por las esquinas y los bordes, así como un cluster de gran tamaño en el centro.

Para calcular el estadístico de Hopkins, generamos unos cuantos valores de *m* que sean proporcionales al número de observaciones de nuestros datos.
```{r hopkins}
set.seed(100)
...
El estadístico de Hopkins nos puede indicar la tendencia de agrupación de los datos, y cuanto más cercano a 1 sea el estadístico, más agrupación tienen los datos.
...
Opciones:
- Código de get_clust_tendency: entender y cambiar para que acepte otras distancias.
- Entender cómo se hace el estadístico de Hopkins y programar desde 0 el algoritmo.
...
Los resultados para el coeficiente Silhouette indican que el número óptimo de clusters es 2 clusters, que parece muy poco para solo `r nrow(beisbol2)` observaciones. Si observamos la variabilidad intra-cluster de los otros números de cluster, vemos que no hay mucha diferencia entre ellos (excluyendo k=1), por lo que entre ellos, podemos coger en el que se crea el codo. Fijandonos en el segundo gráfico, vemos que el codo se crea en K=5, por lo que cogeremos 5 clusters.

Creamos a continuación los 6 clusters con el modelo jerárquico y el método de Ward. Dado que el número de observaciones lo permite, generaremos el dendrograma para visualizar la agrupación de los jugadores
...
# ¿Se agrupan todos los jugadores de la Liga A en el mismo cluster?
b_leagueA = beisbol[,tabla$LeagueA == 1]
which(grupos1 == 1)
```
...
Sin embargo, cuando observamos el dendrograma y el número de observaciones por cluster, vemos que este método propociona clusters más desequilibrados: dos de ellos con solo dos observaciones y otro muy heterogéneo con muchas observaciones: `r max(table(grupos2))`.
