...
El objetivo es encontrar grupos de cereales con similar contenido nutricional. Por ello, utilizaremos para el clustering solo los parámetros nutricionales. Después, ya trataremos de relacionar los clusters obtenidos con la variable *Salary*  o con otras variables no consideradas en el análisis. 
...
```
El mapa de color nos muestra que puede haber entre 6 y 8 clusters (grupo de jugadores de béisbol (beisbolistas)). Por el centro vemos 3 azules de un tamaño mayor, y el resto son más o menos iguales.
...
Vemos cómo el cluster 4 es el que más distancia coge con el resto, sin embargo el cluster 5 se solapa casi totalmente con el resto de clustershelo. 
...
Efectivamente, ahora los cluster 3 y 5 se separan perfectamente en el PCA al representar la primera y tercera componente principal.
...
```
**Repetir el proceso quitando a los jugadores que generan clusters por sí mismos (Pete Rose y Reggie Jackson)**
...
beisbol3 = beisbol2[-c(233, 243),]

midist3 <- get_dist(beisbol3, stand=FALSE, method='euclidean')
...
**Las medidas de index que no funcionan son: ccc hasta friedman (incluidas). Probar a hacer res.nbclust quitando solo esos índices. Se puede usar clValid, como se hace por bajo en las validaciones del modelo.**
